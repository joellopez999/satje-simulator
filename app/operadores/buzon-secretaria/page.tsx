...
export default function BuzonSecretariaPage() {
  ...
  const loadSolicitudes = async () => {
    try {
      setIsLoading(true)
      // Fetch activities of type 'otros' with metadata.solicitud_secretaria = true
      // We can use the existing search API or a more specific query if needed.
      // For now, let's fetch all processes and filter client-side as per the pattern in other pages,
      // or better, let's use a direct query if possible.
      // Given the current API structure, we might need to fetch processes and extract activities.
      ...
      const allSolicitudes = processes.flatMap((process: any) =>
        process.expedientes?.flatMap((expediente: any) =>
          expediente.actividades?.filter((actividad: any) =>
            actividad.metadata?.solicitud_secretaria === true
          ).map((actividad: any) => ({
            id: actividad.metadata.solicitud_id || actividad.id, // Use activity ID as fallback
            original_activity_id: actividad.id,
            titulo_providencia: actividad.metadata.titulo_providencia,
            numero_causa: actividad.metadata.numero_causa || process.numero_causa,
            instrucciones: actividad.metadata.instrucciones,
            estado: actividad.metadata.estado || 'pendiente',
            fecha_solicitud: actividad.metadata.fecha_solicitud || actividad.fecha_creacion,
            solicitado_por: actividad.metadata.solicitado_por,
            solicitado_por_id: actividad.metadata.solicitado_por_id,
            proceso_id: actividad.metadata.proceso_id || process.id,
            expediente_id: expediente.id,
            fecha_completada: actividad.metadata.fecha_completada,
            completada_por: actividad.metadata.completada_por
          })) || []
        ) || []
      ).sort((a: any, b: any) => new Date(b.fecha_solicitud).getTime() - new Date(a.fecha_solicitud).getTime())
  ...
  const handleMarcarCompletada = async (solicitudId: string) => {
    if (!confirm('¿Está seguro de que desea marcar esta solicitud como completada?\n\nEsta acción no se puede deshacer.')) {
      return
    }

    try {
      // Find the actual activity ID. If we mapped it correctly, it should be in original_activity_id or id
      const solicitud = solicitudes.find(s => s.id === solicitudId)
      if (!solicitud) return

      const activityId = solicitud.original_activity_id || solicitud.id

      // Update the activity metadata via API
      // We can use the dispatch API or a generic update API. 
      // Since we don't have a generic update API exposed yet, we might need to use the dispatch one 
      // or create a new one. 
      // However, the dispatch API updates specific fields.
      // Let's check if we can use the dispatch API to update metadata.
      // The dispatch API merges metadata.
      ...
      // We can reuse the dispatch endpoint if it supports generic metadata merge
      // Looking at `app/api/activities/dispatch/route.ts` (I recall it merges metadata)
      ...
      await loadSolicitudes() // Reload to refresh UI
